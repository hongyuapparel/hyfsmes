...
### 问题反馈处理
用户是代码小白，无法判断根因。收到 bug 反馈时：
- **证据不足不猜结论**：先说明不确定的点，再主动列出需要的信息（控制台报错、Network 截图、复现步骤、后端日志等）
- **修复后必须验证**：说明验证了什么、结果是什么；无法自动验证时告知用户需要做哪些操作
- **不能确认修复就不说"修复完成"**：只能说"已按分析修改，请按以下步骤验证"
- **同一问题修复超过 2 次未解决**：停止猜测，列出已排除方向和仍需的新证据
