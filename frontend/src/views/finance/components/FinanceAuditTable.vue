...
import type { FinanceAudit } from '@/api/finance-control'
...
function changes(log: FinanceAudit) {
  ...
  return Object.entries(fields).filter(([key]) => (key in before || key in after) && display(before[key]) !== display(after[key])).map(([key,label]) => `${label}：${display(before[key])} → ${display(after[key])}`)
}
