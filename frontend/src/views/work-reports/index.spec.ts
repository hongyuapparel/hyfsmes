...
it('过期计划有提醒和筛选，已结束订单单独提示并保持完成操作',async()=>{
 ...
 const w=await open();try{expect(w.text()).toContain('逾期 1 天');expect(w.get('.plan-warning').text()).toBe('待确认');await w.findAll('button').find(b=>b.text()==='逾期待处理 1 项')!.trigger('click');expect(w.get('.plan-table').text()).not.toContain('NOW · B');await w.findAll('button').find(b=>b.text()==='编辑')!.trigger('click');await flushPromises();expect(w.text()).toContain('NOW · B');expect(w.text()).toContain('结束跟进');expect(w.text()).toContain('需要协助')}finally{w.unmount()}
})
