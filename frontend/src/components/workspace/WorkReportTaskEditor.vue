<template>
 <div class="plan-table" :class="{'plan-table-editing':editable,'plan-table-other':section==='other'&&!queue}">
  ...
  <el-table size="small" :data="drafts" row-key="id" class="editable-grid" border :cell-style="reportCellStyle" empty-text="暂无待办" @selection-change="selected=$event">
   ...
   <el-table-column v-if="queue || section!=='other'" label="当前状态 / 事项" column-key="status" align="left" min-width="120"><template #default="{row}"><div class="status-copy"><div v-if="finishedOrders(row).length" class="plan-warning">{{ finishedOrders(row).length===taskOrders(row).length?'订单已结束，请确认安排是否结束':'部分订单已结束，请拆分确认' }}</div><template v-if="queue"><div>{{ queueInfo(row)?.status || queueInfo(row)?.title }}</div><div v-if="queueInfo(row)?.status" class="order-meta">{{ queueInfo(row)?.title }}</div><div class="order-meta queue-meta"><span v-if="queueInfo(row)?.factory">{{ queueInfo(row)?.factory }}</span><span v-if="queueInfo(row)?.quantity!=null">数量 {{ queueInfo(row)?.quantity }}</span></div><div v-if="queueInfo(row)?.remark" class="order-meta">{{ queueInfo(row)?.remark }}</div></template><template v-else>{{ [...new Set(taskOrders(row).map(no=>info(row,no)?.status).filter(Boolean))].join('、') }}</template></div></template></el-table-column>
...
const reportCellStyle=({column}:{column:{columnKey?:string}})=>({background:column.columnKey==='order'||column.columnKey==='status'?'var(--color-bg)':'var(--color-card)',padding:column.columnKey?'0 8px':undefined})
...
.plan-warning {color:var(--el-color-warning-dark-2);font-size:var(--font-size-caption)}
