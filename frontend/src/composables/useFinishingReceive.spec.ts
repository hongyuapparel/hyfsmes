...
describe('useFinishingReceive — 真实收货录入', () => {
  ...
  it('收货超过车缝时按实际数量提交，零数量仍被拦截', async () => {
    ...
    c.receiveDialog.tailReceivedQuantitiesByColor[0].quantities = [10, 20]
    await c.submitReceive()
    expect(registerFinishingReceive).toHaveBeenCalledWith({
      orderId: 1, tailReceivedQty: 30,
      tailReceivedQuantitiesByColor: [{ colorName: '杏色', quantities: [10, 20] }],
    })
    expect(c.receiveDialog.visible).toBe(false)
  })
