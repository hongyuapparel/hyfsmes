...
export function assertColorRowsShape(
  ...
  if (!Array.isArray(rows) || rows.length !== planColorNames.length) {
    throw new BadRequestException(`颜色行数 ${rows?.length ?? 0} 与订单颜色数 ${planColorNames.length} 不一致`);
  }
  for (let i = 0; i < rows.length; i++) {
    const expected = (planColorNames[i] ?? '').trim();
    const actual = (rows[i]?.colorName ?? '').trim();
    if (actual !== expected) {
      throw new BadRequestException(`第 ${i + 1} 行颜色 "${actual}" 与订单颜色 "${expected}" 不一致`);
    }
    const q = rows[i]?.quantities;
    if (!Array.isArray(q) || q.length !== sizeLen) {
      throw new BadRequestException(`第 ${i + 1} 行（${expected}）尺码数 ${q?.length ?? 0} 与订单 ${sizeLen} 不一致`);
    }
    for (let cellIndex = 0; cellIndex < q.length; cellIndex++) {
      const value = Number(q[cellIndex]);
      if (!Number.isInteger(value) || value < 0) {
        throw new BadRequestException(`第 ${i + 1} 行（${expected}）第 ${cellIndex + 1} 个尺码数量必须为非负整数`);
      }
    }
  }
}
