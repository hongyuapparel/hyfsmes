...
test('report settings: admin only, validation, persistence, conflict, directory scope, no lost plans',async()=>{
 ...
 try {
  ...
  await db.query('INSERT INTO order_ext(order_id,materials) VALUES (?,?)',[fixtureId,JSON.stringify([{materialName:'测试面料',supplierName:'报告验收供应商',purchaseStatus:'pending',purchaseQuantity:12},{materialName:'测试辅料',supplierName:'报告验收供应商',purchaseStatus:'pending',purchaseQuantity:3}])]);
  ...
  const purchasing=await api(purchasePerson.id+'?date='+date);const inProgress=purchasing.data.automatic[0].rows.find(r=>r.orderId===fixtureId&&r.materialIndex===0);assert.equal(inProgress.status,'采购中');assert.equal(inProgress.planKey,ownPending[0].planKey);
  const finished=await fetch('http://127.0.0.1:3013/production/purchase/items/complete/batch',{method:'POST',headers:{Authorization:'Bearer '+admin,'Content-Type':'application/json'},body:JSON.stringify({items:[{orderId:fixtureId,materialIndex:0}]})});assert.equal(finished.status,201,await finished.text());
