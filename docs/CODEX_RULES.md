# CODEX RULES — 项目默认协作规则

本文件是 Codex 在本项目中的默认规则入口。每次任务开始前先读本文件并默认遵守；无需再额外读取 Claude/Cursor 的入口规则文件。

## 1. 项目定位与技术栈

- 项目是服装工贸一体 B2B ERP，面向内部业务流程：订单、客户、商品、仓储、生产、权限等；不是电商发货系统。
...
- 可扩展现有组件时，优先扩展 `props` / `slots` / 配置，不复制一份新实现。
- 抽屉、弹窗、表单区、工具栏、列表操作区等同类交互必须保持结构和视觉一致；特殊偏离先说明原因并获确认。
...
- 目标是项目不出现 `any` 和新增技术债；所有新代码、本次触碰代码禁止 `: any`、`as any`、`any[]`、`Record<string, any>`、`Promise<any>` 等写法，必须使用具体类型或 `unknown` + 类型守卫。
...
- 文件接近警戒线必须拆分，超过警戒线不得继续堆代码：`views/*.vue` 300/500 行，`components/*.vue` 200/350 行，`composables/use*.ts` 150/250 行，`stores/*.ts` 150/200 行，backend `*.service.ts` 300/500 行。
...
低优先级规则与高优先级冲突时，按高优先级执行，并在回复中说明。
