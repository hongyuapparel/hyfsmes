...
- [ ] **Step 2: 切换 `order-mutation.service.ts`**
...
- [ ] **Step 3: 切换 `order-lifecycle.service.ts`**

把 `:146-154` 同样的 7 行段替换为 1 行调用，import 同上。

- [ ] **Step 4: 切换 `order-status.service.ts`**
...
- [ ] **Step 5: 切换 `production-purchase.service.ts:117-122`**
...
- [ ] **Step 2: 在 `registerPurchase` 末尾追加日志写入**
...
- [ ] **Step 3: 在 `registerPurchaseBatch` 末尾追加批量日志**
