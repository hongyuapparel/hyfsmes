...
- [ ] Step 1 写 3 个 entity（字段与上方 SQL 一一对应；JSON 列用 `@Column({ type: 'json', nullable: true })`，类型 `string[] | null` / `Record<string, number> | null`，照 `order-ext.entity.ts` 写法）
- [ ] Step 2 写 `backend/scripts/create-packing-list-tables.sql`（内容即上方 SQL，文件头注释一行用途）
- [ ] Step 3 `main.ts` 加 `ensurePackingListTables(dataSource)`：三段 `CREATE TABLE IF NOT EXISTS`（照 `ensureInventoryOperationLogTables` 行 156-188 模式），在启动序列（行 302-328 区域）加调用
- [ ] Step 4 `seed-permissions.ts` PERMISSIONS 数组「库存管理」分组内（行 33 后）插入 menu_inventory_packing 条目
- [ ] Step 5 `cd backend && npm run build` 通过
- [ ] Step 6 commit `feat(packing): 装箱单数据表与权限种子`

### Task 2：后端模块 CRUD

**Files:** 新建 module/controller/service；修改 `app.module.ts`。

- [ ] Step 1 `packing-lists.service.ts`：
...
- [ ] Step 2 controller：`@Controller('packing-lists')` + `@UseGuards(JwtAuthGuard, PermissionGuard)` + `@RequirePermission('/inventory/packing')`，`@CurrentUser()` 取 username（照 inventory-pending.controller 模式）
- [ ] Step 3 module：forFeature 三个新实体 + `Order`、`OrderExt`、`InboundPending`、`FinishedGoodsStock`、`FinishedGoodsStockColorImage`、`Product`；imports `AuthModule`；app.module 注册
- [ ] Step 4 `npm run build` 通过
- [ ] Step 5 commit `feat(packing): 装箱单后端 CRUD`

### Task 3：后端选货接口

**Files:** 新建 `packing-lists-pickable.service.ts`；controller 加两个 GET。

- [ ] Step 1 `getPendingPickable({customerName, keyword, page, pageSize})`：
...
- [ ] Step 2 `getFinishedPickable(...)`：`finished_goods_stock` 按 customer_name 筛；逐色拆 `colorSizeSnapshot`；颜色图查 `finished_goods_stock_color_images`；无 snapshot 同上降级
- [ ] Step 3 controller 加 `GET pickable/pending`、`GET pickable/finished`
- [ ] Step 4 build + commit `feat(packing): 装箱单选货查询接口`

### Task 4：后端发货

**Files:** 新建 `packing-lists-ship.service.ts`；修改 `inventory-pending.module.ts`（exports）、`packing-lists.module.ts`（imports 两模块）。

- [ ] Step 1 `inventory-pending.module.ts` 加 `exports: [InventoryPendingService]`；packing module imports `InventoryPendingModule`、`FinishedGoodsStockModule`
- [ ] Step 2 `ship(id, username)` 逻辑：
  ```
...
- [ ] Step 3 先建两个最小占位 .vue（仅标题）保证路由编译；`cd frontend && npm run build` 通过
- [ ] Step 4 commit `feat(packing): 装箱单前端路由与 API 封装`

### Task 6：列表页 packing.vue

- [ ] Step 1 筛选栏（客户关键字/状态/日期范围，照 pending.vue 工具栏密度）+ `新建装箱单` 按钮 → push edit 路由
- [ ] Step 2 表格列：单号/客户/业务员/款号摘要（items 去重 styleNo 首个+N款）/箱数/件数/总重/状态 tag/装箱日期/操作（编辑或查看、箱贴、导出、删除-仅草稿，删除走 ElMessageBox.confirm）
- [ ] Step 3 操作「箱贴/导出」直接 getDetail 后复用 Task 9/10 的组件与 util
- [ ] Step 4 build + commit `feat(packing): 装箱单列表页`
...
- [ ] Step 1 `usePackingGridRows.ts` 纯逻辑（先写 spec 后写实现，照 `useOrderSizeInfo.spec.ts` 模式）：
...
- [ ] Step 2 `npx vitest run src/composables/usePackingGridRows.spec.ts` 通过
- [ ] Step 3 `PackingGrid.vue`：`<el-table :data="flatRows" class="editable-grid" :span-method="spanMethod">`
...
- [ ] Step 4 `PackingGoodsPickerDialog.vue`：AppDialog(width 900) + el-tabs（待仓处理/成品库存）+ 关键字搜索 + 分页表（款号/订单号/颜色图/颜色/各码可发/合计/已分配提示），多选 → emit `picked(lines)`；已选过的行打标
- [ ] Step 5 `usePackingListEdit.ts`：路由参数加载详情/新建空单（默认 1 空箱、pack_date 今天、size_headers []）；表头表单（客户=getAllCustomerCompanyOptions 的 el-select filterable allow-create、业务员=getUsers el-select filterable allow-create、PO、整单备注、公司名 el-switch）；`save()`（create/update 后回列表或留页提示）；客户切换且已有来源行时 ElMessageBox 提醒清空来源行
- [ ] Step 6 `packing-edit.vue` 组装；选货行进箱默认 sizeQuantities=剩余可发（pickable 值减 allocationBySource）
- [ ] Step 7 手工加行：box 行操作「+手工行」直接插空行（source_type manual，全字段可编）
- [ ] Step 8 `npm run build`（vue-tsc）通过 + commit `feat(packing): 装箱单编辑页与选货`

### Task 8：发货动作

- [ ] Step 1 编辑页「确认发货」：先 `validateAgainstPicked` 前端拦截超发 → ElMessageBox.confirm（列箱数/件数）→ `save()` → `shipPackingList(id)` → 成功后状态变只读（所有输入 disabled、按钮区换成 箱贴/导出/返回）
- [ ] Step 2 已发货单进编辑页全只读（detail.status 驱动）
- [ ] Step 3 build + commit `feat(packing): 装箱单确认发货`

### Task 9：箱贴 PackingLabelPrint.vue

- [ ] Step 1 props：`detail: PackingListDetail`、`v-model`(visible)。AppDialog 全宽预览，每箱一张 label（方案 D 版式）：
...
- [ ] Step 2 可改字：所有文本节点 `contenteditable="true"`（只改 DOM 不回写数据）；箱多选 checkbox 决定打印哪些
- [ ] Step 3 打印：组件内 `@media print` —— `body * visibility:hidden; .packing-label-print-area, .packing-label-print-area * visibility:visible; position:absolute; inset:0`，每张 label `page-break-after: always`、宽 210mm 内边距留白（照 orders/detail.css 的 a4 尺寸基准）；按钮 `window.print()`
- [ ] Step 4 列表页/编辑页接入；build + commit `feat(packing): 箱贴一键生成与打印`

### Task 10：Excel 导出 packing-export.ts

- [ ] Step 1 `exportPackingListExcel(detail)`：`XLSX.utils.aoa_to_sheet` 组装（贴近公盘模板）：
...
- [ ] Step 2 列表页+编辑页接「导出 Excel」；build + commit `feat(packing): 装箱单导出双语 Excel`

### Task 11：收尾验证

- [ ] Step 1 根目录前后端完整 build 各跑一次；`npm run check:el-radio-value`
- [ ] Step 2 `npx vitest run`（前端既有 + 新 spec）
- [ ] Step 3 `scripts/start.ps1` 起本地，过冒烟：菜单出现 → 新建（手填客户+手工行+插码列）→ 保存草稿 → 重开 → 选货（两来源）→ 确认发货 → 待仓/成品两页核对扣减与出库记录 → 箱贴打印预览 → Excel 导出打开核对
- [ ] Step 4 `git status` 核对改动面；docs/PROJECT_CONTEXT.md 更新记录一行
- [ ] Step 5 commit `docs: 装箱单功能说明`
