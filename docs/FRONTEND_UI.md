...
- 同一动作沿用同一叫法、按钮样式和所在区块的操作位置。单条与批量入口可以并存，但须明确作用对象；不能为了避免重复而让单选后没有可用操作。
- 先利用现有空间和组件能力解决尺寸、间距问题，不为放大图片另加一套布局。视觉修改须实际检查完整页面及窄屏/矮屏，详见 [`AGENT_WORKFLOW.md`](AGENT_WORKFLOW.md) 第 3 节。
...
| 层级 | 变量 | 用途 |
|------|------|------|
| 主标题 | `--font-size-title` | 页面主标题，16px；与次标题通过字重和间距区分 |
| 次标题 | `--font-size-subtitle` | 弹窗、抽屉、卡片、区块标题，16px |
| 正文 | `--font-size-body` | 表单、按钮、菜单、页签、表格及表格内输入，14px |
| 紧凑文字 | `--font-size-caption` | 辅助说明、日期及元数据，12px |
...
- 表头、只读单元格及单元格内输入控件统一使用 `--font-size-body`，不允许输入状态改变字号。
