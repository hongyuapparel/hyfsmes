...
后端和完整部署脚本会从 `backend/.env` 自动读取实际 `PORT`，并在 PM2 重启后检查该端口的 `/health` 与 `/health/db`；未配置时才默认使用 3000。健康检查默认最多等待 120 秒，以覆盖生产环境启动时的结构检查和种子数据检查。健康检查通过时不打印 PM2 错误日志；只有新进程或数据库未就绪时，才输出最后一次健康响应与最近 50 行 PM2 历史日志并以失败状态退出。PM2 日志会明确标注为历史上下文，不能仅凭其中的旧日期错误判断本次部署失败原因。
