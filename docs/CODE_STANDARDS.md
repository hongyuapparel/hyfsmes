...
- 公共组件问题先追查实例、状态和生命周期；不能用页面硬编码隐藏、固定宽度或刷新来掩盖。修复共享逻辑后按 `AGENT_WORKFLOW.md` 验证直接受影响的使用场景。
- `null`、空值和 0 按字段语义处理，不把“清空”误当“未修改”；数量和业务事实不得用未经确认的计划值兜底，实际数据边界见 `AGENT_WORKFLOW.md`。
