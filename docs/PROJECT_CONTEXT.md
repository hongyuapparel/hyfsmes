...
- `2026-05-01`：成品库存列表移除订单号筛选与订单号列，订单号仅作为来源信息保留在详情操作记录；同 SKU 库存父行合并与分页口径不再受订单号影响。
- `2026-05-02`：成品库存统一尺码排序口径（XXS/XS/S/M/L/XL/2XL...），详情抽屉改为“基本信息 / 库存明细 / 操作记录”页签，父行和子行共用同一详情抽屉，选中现有库存新增时按同组颜色尺码结构快速加数。
- `2026-05-05`：成品库存新增库存入口由弹窗改为右侧抽屉，并与库存详情复用同一套颜色图片与码数明细矩阵；新增可编辑数量并支持选中 SKU 后快速加数/加颜色/加尺码，详情数量只读且不承担入库，只允许编辑现有 SKU、基础信息、产品图、颜色图片、尺码名称与单价，入库/出库/详情修改统一进入操作记录。
...
- `2026-05-15`：尾部管理支持「分批入库」：同一订单可多次「登记入库」，按本次量累加并写入 inbound_pending 批次行（含 operator_username / batch_no）；登记弹窗改为「部分入库 / 全部入库」双按钮；列表「尾部入库数」加`部分入库`标签；详情抽屉新增「批次记录」时间轴；已完成订单的修正路径保持原覆盖式语义。
- `2026-07-11`：订单列表新增回收站视图与 `orders_restore` 恢复；超级管理员可「强制改状态」（`POST /orders/:id/admin/force-status`，默认不重置工序）；admin 保存订单时跳过流程自动重算。
- `2026-07-15`：车缝纠错分单与数量合并进 `POST .../sewing/items/:id/edit`（仅需 `production_admin_edit`，不再先调 assign）；强制改状态的 `resetSubsequent` 暂拒用，防止重置领料状态却不回滚库存。
