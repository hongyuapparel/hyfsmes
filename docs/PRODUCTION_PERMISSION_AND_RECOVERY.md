...
在「设置 → 角色权限」中按需勾选（相互独立）：

| 权限码 | 用途 |
|--------|------|
| `orders_force_status` | 强制改订单主状态（默认只改状态，不自动清生产数据；`resetSubsequent` 暂不开放，以免重置领料状态却不回滚库存） |
| `production_admin_edit` | 编辑各生产环节已提交 / 已完成的登记数据（车缝纠错含分单信息与数量，同接口一次写入，不依赖 `production_sewing_assign`） |

若订单被误点到下一环节、或主状态与业务不符：

1. 订单列表或编辑页 **强制改状态**（需 `orders_force_status`）。
2. 选择目标状态并填写原因；**不**默认重置工序。
3. 生产登记填错：用持有 `production_admin_edit` 的账号在对应生产页改正数据。
4. 若子工序仍为完成，约 2 分钟内定时调和可能再次推进主状态，需同步改正工序或再改状态。
5. 操作均写操作记录；辅料出库 / 成品入库不自动回滚。

回收站误删：订单列表 → **回收站** → 勾选 → **恢复**（需 `orders_restore` 权限）。

### 3.1 子记录完成但主订单未推进（向前调和）
...
2. 子记录完成但主订单状态没推进：
   - 后台定时任务每 2 分钟会按 `order_status_transitions` 流程配置自动调和。
...
如果不能确认目标状态，不要手工更新，先修流程链路配置；**回退状态请优先用超级管理员「强制改状态」**，手工 SQL 仅作最后手段。
...
- `orders_delete`：订单删除（移入回收站）
- `orders_restore`：从回收站恢复订单
- `orders_force_status`：强制改订单主状态
- `orders_review`：订单审核通过/驳回
- `orders_cost_submit`：订单成本提交、成本模板保存入口之一

后端生产动作权限：

- `production_admin_edit`：编辑各环节已提交 / 已完成的生产登记（纠错；采购外购登记/领料备注纠错（不重新扣库存）/纸样样衣图/裁床/车缝/尾部修订入库；纸样物料本身可改无需此权；工艺无独立登记字段，完成后勿重复点完成）
